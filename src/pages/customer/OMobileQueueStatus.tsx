 ...
 * 
 * Reference Mockup: mobile_customer_queue_status.png
 * Path: c:\example\IDE\10.Project\2026\03.AdaQueue\docs\mockup\mobile_customer_queue_status.png
 */
const OMobileQueueStatus: React.FC = () => {
  ...
  const getStatusText = () => {
    switch (ticket.status) {
      ...
      case 'NEAR': return 'Almost Your Turn';
      case 'CALLED': return 'Please Go to Counter 3';
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 font-sans flex flex-col items-center relative overflow-hidden transition-colors duration-200">
      ...
      {/* Background Decor */}
      <div className={`absolute top-0 inset-x-0 h-64 ${getStatusColor()} rounded-b-[3rem] shadow-lg transition-colors duration-500 z-0`}></div>
      ...
      <div className="w-full max-w-md px-6 pt-12 pb-6 z-10 flex-1 flex flex-col">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl p-8 flex flex-col items-center text-center relative overflow-hidden transition-all duration-300">
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wider text-sm mb-2">Your Ticket Number</p>
          <div className="text-7xl font-black text-gray-900 dark:text-white tracking-tighter mb-4">
            {ticket.number}
          </div>

          <div className={`px-4 py-2 rounded-full text-sm font-bold text-white mb-8 shadow-md ${getStatusColor()} transition-colors duration-500`}>
            {getStatusText()}
          </div>
          ...
          <div className="w-full grid grid-cols-2 gap-4 mb-6">
            <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-2xl">
              <p className="text-blue-600 dark:text-blue-400 text-xs font-bold uppercase mb-1">Queues Ahead</p>
              <p className="text-3xl font-bold text-gray-800 dark:text-white">{ticket.queuesAhead}</p>
            </div>
            <div className="bg-purple-50 dark:bg-purple-900/20 p-4 rounded-2xl">
              <p className="text-purple-600 dark:text-purple-400 text-xs font-bold uppercase mb-1">Est. Wait Time</p>
              <p className="text-3xl font-bold text-gray-800 dark:text-white">{ticket.estimatedWait} <span className="text-sm font-normal text-gray-500">min</span></p>
            </div>
          </div>
           ...
           {isExpanded && (
             <div className="mt-4 bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm text-left animate-fade-in-down">
               <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                 <span className="text-gray-500 dark:text-gray-400 text-sm">Service Type</span>
                 <span className="font-medium text-gray-800 dark:text-white text-sm">Consultation</span>
               </div>
               <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
                 <span className="text-gray-500 dark:text-gray-400 text-sm">Date</span>
                 <span className="font-medium text-gray-800 dark:text-white text-sm">{new Date().toLocaleDateString()}</span>
               </div>
               <div className="flex justify-between py-2">
                 <span className="text-gray-500 dark:text-gray-400 text-sm">Location</span>
                 <span className="font-medium text-gray-800 dark:text-white text-sm">Floor 3, Zone A</span>
               </div>
             </div>
           )}
        </div>
