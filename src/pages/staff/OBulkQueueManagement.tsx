...
const generateMockQueues = (workflow: IWorkflowDefinition | null): IQueueItem[] => {
    if (!workflow || !workflow.servicePoints || workflow.servicePoints.length === 0) return [];
    if (!workflow.serviceGroups) return [];

    const queues: IQueueItem[] = [];
    const tiers: ('Standard' | 'Gold' | 'Platinum')[] = ['Standard', 'Gold', 'Platinum'];
    const channels: ('Walk-in' | 'Mobile' | 'Voice' | 'Chat')[] = ['Walk-in', 'Mobile', 'Voice', 'Chat'];
    const statuses: ('WAITING' | 'SERVING' | 'HOLD' | 'ASSIGNED')[] = ['WAITING', 'SERVING', 'HOLD', 'ASSIGNED'];

    // Generate 15 mock items distributed across service points
    for (let i = 0; i < 15; i++) {
        const point = workflow.servicePoints[Math.floor(Math.random() * workflow.servicePoints.length)];
        if (!point) continue;

        // Find group for this point (first one)
        const groupCode = point.serviceGroups && point.serviceGroups.length > 0 ? point.serviceGroups[0] : '';
...
const MOCK_LOGS: IActivityLog[] = [
  { id: '1', user: 'Admin_Sarah', action: 'Bulk Cancel', details: 'Performed Bulk Cancel on 15 tickets from Zone-A (Technical).', timestamp: '2 MINS AGO', type: 'danger' },
  { id: '2', user: 'Staff_04', action: 'Transferred', details: 'Transferred 8 items from Sales Queue to Express Lane.', timestamp: '14 MINS AGO', type: 'info' },
  { id: '3', user: 'System', action: 'Bulk Skip', details: 'Bulk Skip applied to 3 timed-out entries in Waitlist-01.', timestamp: '45 MINS AGO', type: 'warning' },
  { id: '4', user: 'Admin_Sarah', action: 'Mass Override', details: 'High Priority flag added to all entries in Crisis Desk.', timestamp: '1 HOUR AGO', type: 'info' },
];
...
const OBulkQueueManagement: React.FC = () => {
  ...
  const fetchWorkflow = async (profileId: string) => {
      ...
      try {
          ...
          if (res.ok) {
              ...
              // Mandatory Two-Step Selection: Do NOT auto-select point
              // Just load the workflow. User must select point next.
          }
      } catch (e) { console.error("Failed to fetch workflow", e); }
    ...
    try {
      ...
      if (res.ok) {
        ...
        const mapped: IQueueItem[] = data.map((q: any) => {
          // Parse data if missing (TypeORM getter issue workaround)
          ...
          return {
            id: q.docNo,
            ticketNo: (groupCode ? `${groupCode}-${String(q.queueNo || q.docNo).padStart(3,'0')}` : String(q.queueNo || q.docNo)),
            customerName: q.customerName || 'Guest',
            customerTier: 'Standard',
            channel: 'Walk-in',
            serviceType: groupName, // Display Name
            servicePoint: qData.counter || '-',
            waitTime: '—',
            status: q.status || 'WAITING',
            serviceGroupCode: groupCode, // For Filtering
  ...
  const filteredQueues = queues.filter(q => {
      if (filterGroup !== 'ALL' && q.serviceGroupCode !== filterGroup) return false;
      if (filterPoint !== 'ALL' && q.servicePointCode !== filterPoint) return false;
      return true;
  });
    ...
    (async () => {
      try {
        const res = await fetch(apiPath('/api/staff/queue/bulk'), {
          ...
          body: JSON.stringify({ action: action.toLowerCase().replace('bulk ', ''), docNos: selectedIds })
        });
        if (res.ok) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {profiles.map((profile) => (
                        ...
                        >
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-white mb-2 group-hover:text-blue-400 transition-colors">{profile.name}</h3>
    ...
    <div className="flex h-full bg-gray-50 dark:bg-gray-900 overflow-hidden relative transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 md:px-8 py-4 md:py-6 flex-shrink-0 transition-colors duration-200">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
                 Bulk Queue Actions
                 {loading && <span className="text-sm font-normal text-gray-500">(Loading...)</span>}
              </h1>
              <p className="text-sm md:text-base text-gray-500 dark:text-gray-400 mt-1 hidden md:block">Manage multiple customer tickets based on Workflow Configuration.</p>
            </div>
          ...
          <div className="mt-4 md:mt-6 flex flex-col xl:flex-row gap-4 justify-between">
            ...
            <div className="flex gap-2 md:gap-3 overflow-x-auto pb-2 md:pb-0 items-center">
              ...
              <button className="px-3 md:px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 font-medium flex items-center gap-2 text-sm whitespace-nowrap transition-colors" onClick={() => fetchWorkflow(selectedProfileId)}>
              ...
              <div className="relative">
                 ...
                 >
                    ...
                    {workflow?.servicePoints
                        .filter(p => filterGroup === 'ALL' || (p.serviceGroups && p.serviceGroups.includes(filterGroup)))
                        .map(p => (
        ...
        {selectedIds.length > 0 && (
          <div className="bg-blue-600 dark:bg-blue-700 text-white px-4 md:px-8 py-3 flex flex-col md:flex-row items-center justify-between shadow-md animate-fade-in-down gap-3 z-10">
            <div className="flex items-center gap-4">
              <span className="font-bold text-lg">{selectedIds.length} ITEMS</span>
              <span className="text-blue-100 text-sm border-l border-blue-500 pl-4 hidden md:inline">Perform bulk actions on selection.</span>
            </div>
        ...
        <div className="flex-1 overflow-auto p-4 md:p-8">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col min-w-[800px] lg:min-w-0 transition-colors duration-200">
             {/* Table container for horizontal scroll on mobile if strictly needed, but min-w forces scroll on parent */}
                    ...
                    <tr>
                      <th className="px-6 py-4 w-12">
                        <input 
                          ...
                          checked={selectedIds.length === filteredQueues.length && filteredQueues.length > 0}
                      ...
                      <th className="px-6 py-4">Service Point</th>
                    ...
                    {filteredQueues.length === 0 ? (
                        <tr>
                            <td colSpan={7} className="px-6 py-12 text-center text-gray-400">
                                {workflow ? "No queues found matching filters." : "Please select a profile to load data."}
                            </td>
                        </tr>
                    ) : filteredQueues.map((item) => (
                      <tr key={item.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${selectedIds.includes(item.id) ? 'bg-blue-50/50 dark:bg-blue-900/20' : ''}`}>
                        ...
                        <td className="px-6 py-4 font-mono text-blue-600 dark:text-blue-400 font-bold">{item.ticketNo}</td>
                        <td className="px-6 py-4">
                          <div>
                            <div className="font-bold text-gray-900 dark:text-white">{item.customerName}</div>
                            <div className="text-xs text-gray-500 dark:text-gray-400">Tier: {item.customerTier} / {item.channel}</div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <span className="px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded text-xs text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600">{item.serviceType}</span>
                        </td>
                         <td className="px-6 py-4">
                          <span className="text-sm text-gray-700 dark:text-gray-300">{item.servicePoint}</span>
                        </td>
                        <td className={`px-6 py-4 font-mono font-bold ${parseInt(item.waitTime) > 15 ? 'text-red-500 dark:text-red-400' : 'text-gray-700 dark:text-gray-300'}`}>
                          {item.waitTime}
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <span className={`w-2 h-2 rounded-full ${item.status === 'WAITING' ? 'bg-yellow-400' : 'bg-blue-500'}`}></span>
                            <span className="text-xs font-bold uppercase text-gray-700 dark:text-gray-300">{item.status}</span>
                          </div>
                        </td>
                      </tr>
                    ))}
            ...
            <div className="px-6 py-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center text-gray-500 dark:text-gray-400 text-sm">
              <span>Showing {filteredQueues.length} results</span>
              <div className="flex gap-1">
                 <button className="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700 disabled:opacity-50"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7"></path></svg></button>
                 <button className="px-3 py-1 rounded bg-blue-600 text-white">1</button>
                 <button className="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"></path></svg></button>
              </div>
            </div>
          </div>
        </div>
      ...
      `}>
        <div className="p-4 md:p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          ...
          <button onClick={() => setHistoryOpen(false)} className="lg:hidden text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-white">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 md:space-y-8">
          {MOCK_LOGS.map((log) => (
            <div key={log.id} className="relative pl-6 border-l border-gray-200 dark:border-gray-700">
               <span className={`absolute -left-1.5 top-1 w-3 h-3 rounded-full border-2 border-white dark:border-gray-800 shadow-sm ${
                 log.type === 'danger' ? 'bg-red-500' : log.type === 'warning' ? 'bg-yellow-500' : 'bg-blue-500'
               }`}></span>
               <div className="flex justify-between items-start mb-1">
                 <span className="font-bold text-gray-800 dark:text-gray-200 text-sm">{log.user}</span>
                 <span className="text-xs text-gray-400 dark:text-gray-500">{log.timestamp}</span>
               </div>
               <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                 <span className={`${
                    log.type === 'danger' ? 'text-red-600 dark:text-red-400' : log.type === 'warning' ? 'text-yellow-600 dark:text-yellow-400' : 'text-blue-600 dark:text-blue-400'
                 } font-medium`}>{log.action}</span> {log.details}
               </p>
            </div>
          ))}
        </div>
