...
const ODisplayBoard: React.FC = () => {
  ...
  useEffect(() => {
    if (startupStep !== 'ready' || !selectedProfileId || !selectedBoard) return;
      ...
      try {
        ...
        if (res.ok) {
          ...
          let mapped = data.map((q: any) => {
            ...
            let finalServiceGroup = q.queueType;
            ...
            if (!finalServiceGroup && embedded.kitchenCode) {
              ...
              if (matchedServicePoint && matchedServicePoint.serviceGroups && matchedServicePoint.serviceGroups.length > 0) {
                // เช็คว่ามีกลุ่มบริการของ ServicePoint อันไหนที่ Display Board นี้ดูได้บ้าง
                const validGroup = matchedServicePoint.serviceGroups.find(
                  (sgCode: string) => selectedBoard?.visibleServiceGroups?.includes(sgCode)
                );

                if (validGroup) {
                  finalServiceGroup = validGroup;
                }
              }
            }

            // Fallback
            if (!finalServiceGroup) {
              finalServiceGroup = 'General';
            }
            ...
            return {
              ...
              ticketNo: q.ticketNo || '',
              status: q.status,
              serviceGroup: finalServiceGroup,
              ...
              kitchenCode: embedded.kitchenCode
            };
          });

          // Filter queues for this board based on its valid service groups
          // Note: Since we now map the correct serviceGroup based on servicePoints + board visibility,
          // the board will automatically render them correctly via visibleGroupCodes filters down below.
          // We only need to filter out queues that don't belong to any visibleServiceGroups of this board entirely.
          // However, the standard logic inside the render function already does the filtering:
          // queues={queues.filter(q => visibleGroupCodes.includes(q.serviceGroup))}
          // So we don't necessarily have to strictly filter by kitchenCode here anymore.

          if (selectedBoard?.kitchenCode) {
            // (Legacy Support) if board still explicitly has kitchenCode, we might filter it.
  ...
  const loadWorkflowForProfile = async (profileId: string): Promise<IDisplayBoardDefinition[]> => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Store serviceGroups, servicePoints and businessType for later use
        if (workflow.serviceGroups) {
          setServiceGroups(workflow.serviceGroups);
        }
        if (workflow.servicePoints) {
          setServicePoints(workflow.servicePoints);
        }
