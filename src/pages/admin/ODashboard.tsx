      ...
      <div className="w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            ...
            {
              label: t("dashboard.stats.avgWaitTime"),
              value: "14m",
              change: "-2m",
              color: "green",
            },
            {
              label: t("dashboard.stats.activeCounters"),
              value: "8/10",
              change: "80%",
              color: "purple",
            },
            {
              label: t("dashboard.stats.servedToday"),
              value: "856",
              change: "+5%",
              color: "indigo",
            },
            ...
            >
              ...
              <div className="flex items-end justify-between mt-2">
                ...
                <span
                  className={`text-sm font-bold ${stat.change.startsWith("+") ? "text-green-500" : "text-red-500"} bg-opacity-10 px-2 py-1 rounded`}
                >
                  {stat.change}
                </span>
              </div>
              <div
                className={`mt-4 h-1 w-full bg-${stat.color}-100 dark:bg-${stat.color}-900/30 rounded-full overflow-hidden`}
              >
                <div
                  className={`h-full bg-${stat.color}-500 w-2/3 rounded-full`}
                ></div>
              </div>
            </div>
          ))}
          ...
          <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-gray-800 dark:text-white">
                {t("dashboard.chart.trafficOverview")}
              </h3>
              <select className="bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-1 text-sm text-gray-600 dark:text-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option>{t("dashboard.chart.today")}</option>
                <option>{t("dashboard.chart.thisWeek")}</option>
                <option>{t("dashboard.chart.thisMonth")}</option>
              </select>
            </div>
              ...
              <div className="space-y-3">
                <div className="p-3 bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 rounded-r-lg">
                  <p className="text-xs font-bold text-red-600 dark:text-red-400">
                    {t("dashboard.alerts.highWaitTitle")}
                  </p>
                  <p className="text-xs text-red-500 dark:text-red-300 mt-1">
                    {t("dashboard.alerts.highWaitDesc")}
                  </p>
                </div>
                <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-500 rounded-r-lg">
                  <p className="text-xs font-bold text-yellow-600 dark:text-yellow-400">
                    {t("dashboard.alerts.paperLowTitle")}
                  </p>
                  <p className="text-xs text-yellow-500 dark:text-yellow-300 mt-1">
                    {t("dashboard.alerts.paperLowDesc")}
                  </p>
                </div>
              </div>
            </div>
