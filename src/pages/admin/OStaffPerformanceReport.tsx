...
const OStaffPerformanceReport: React.FC = () => {
    ...
    const fetchAll = async () => {
      ...
      try {
        const [sumRes, heatRes, staffRes] = await Promise.all([
          ...
          fetch(
            apiPath(`/api/performance/staff?page=${page}&pageSize=${pageSize}`),
          ),
        ]);
  ...
  const staffData = staffRows.map((r) => ({
    ...
    aht: r.avgHandlingTimeSec
      ? new Date(r.avgHandlingTimeSec * 1000).toISOString().substring(14, 19)
      : "00:00",
  ...
  return (
    <div className="min-h-full bg-gray-50 dark:bg-gray-900 p-6 md:p-8 font-sans transition-colors duration-200">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={async () => {
              try {
                const res = await fetch(apiPath("/api/performance/export"), {
                  method: "POST",
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        {[
          t("report.filters.branch"),
          t("report.filters.zone"),
          t("report.filters.staffRole"),
          t("report.filters.date"),
        ].map((filter, idx) => (
          <div
            key={idx}
            className="bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700 flex justify-between items-center text-gray-700 dark:text-gray-300 font-medium cursor-pointer hover:border-blue-500 transition shadow-sm"
          >
            <span>{filter}</span>
            <svg
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          {
            label: t("report.summary.overallSla"),
            value: summary ? `${summary.overallSlaCompliancePct}%` : "-",
            isPositive: true,
            icon: "🛡️",
            color: "blue",
          },
          {
            label: t("report.summary.totalTickets"),
            value: summary ? `${summary.totalTicketsHandled}` : "-",
            isPositive: true,
            icon: "🎫",
            color: "indigo",
          },
          {
            label: t("report.summary.avgWaitTime"),
            value: summary
              ? new Date((summary.avgWaitTimeSec || 0) * 1000)
                  .toISOString()
                  .substring(14, 19)
              : "-",
            isPositive: true,
          ...
          {
            label: t("report.summary.avgCsat"),
            value:
              summary && summary.avgCsatScore != null
                ? `${summary.avgCsatScore}/5.0`
                : "-",
            isPositive: true,
            icon: "⭐",
            color: "yellow",
          ...
          <div
            key={idx}
            className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm relative overflow-hidden group hover:shadow-md transition-all"
          >
            <div className="flex justify-between items-start mb-4">
              ...
              <div
                className={`p-2 rounded-lg bg-${stat.color}-100 dark:bg-${stat.color}-900/30 text-${stat.color}-600 dark:text-${stat.color}-400`}
              >
            ...
            {/* Decorative bar */}
            <div
              className={`absolute bottom-0 left-0 w-full h-1 bg-${stat.color}-500 opacity-0 group-hover:opacity-100 transition-opacity`}
            ></div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mb-8">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">
            {t("report.heatmap.title")}
          </h3>
          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
            <span>{t("report.heatmap.lowTraffic")}</span>
            ...
            <span>{t("report.heatmap.peakLoad")}</span>
          </div>
        </div>
          ...
          <div className="min-w-[800px]">
            {days.map((day, dIdx) => (
              <div key={day} className="flex items-center mb-2">
                <div className="w-12 text-sm text-gray-500 dark:text-gray-400 font-medium">
                  {day}
                </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-900/50 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700">
                <th className="px-6 py-4">{t("report.table.staffMember")}</th>
                <th className="px-6 py-4">{t("report.table.roleCounter")}</th>
                <th className="px-6 py-4">
                  {t("report.table.ticketsHandled")}
                </th>
                <th className="px-6 py-4">{t("report.table.aht")}</th>
                <th className="px-6 py-4">{t("report.table.slaCompliance")}</th>
                <th className="px-6 py-4">{t("report.table.slaBreaches")}</th>
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`text-sm font-bold ${staff.sla >= 90 ? "text-green-500" : staff.sla >= 80 ? "text-yellow-500" : "text-red-500"}`}
                      >
                        {staff.sla}%
                      </span>
                    </div>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center text-sm text-gray-500 dark:text-gray-400">
          <span>
            {meta
              ? t("report.pagination.showing", {
                  from: (page - 1) * 20 + 1,
                  to: Math.min(page * 20, meta.pagination.totalItems),
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded hover:bg-gray-50 dark:hover:bg-gray-700 transition"
            >
              {t("report.pagination.previous")}
            </button>
