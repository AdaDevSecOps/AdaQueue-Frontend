 ...
 */
export const OIssueTicket: React.FC<IIssueTicketProps> = ({
  category,
  onConfirm,
  onBack,
}) => {
    ...
    const loadConfig = async () => {
      try {
        ...
        if (activeProfileId) {
          ...
          const workflowJson = localStorage.getItem(
            `adaqueue_workflow_${activeProfileId}`,
          );
          if (workflowJson) {
            ...
            if (storedKioskCode) {
              kioskConfig = workflow.kiosks?.find(
                (k) => k.code === storedKioskCode,
              );
            ...
            const group = workflow.serviceGroups.find(
              (g) => g.code === category,
            );
            if (group) {
  ...
  const handleSubmit = async () => {
    ...
    const profileId =
      localStorage.getItem("adaqueue_selected_profile") || undefined;
    ...
    <div className="min-h-screen bg-gray-900 text-white font-sans flex flex-col">
      ...
      <div className="flex-1 px-8 py-6 pb-8 flex flex-col lg:flex-row gap-8 lg:gap-16 max-w-7xl mx-auto w-full">
        ...
        <div className="flex-1 flex flex-col">
          <h1 className="text-4xl font-bold mb-8">{t("issueTicket.title")}</h1>

          <div className="mb-8">
            <label className="flex items-center gap-2 text-gray-400 text-lg font-bold mb-3">
              <span>{t("issueTicket.phoneLabel")}</span>
            </label>
        ...
        <div className="w-full lg:w-[450px] flex flex-col">
          ...
          >
            {loading
              ? t("issueTicket.processing")
              : t("issueTicket.confirmTicket")}
          </button>
          ...
          <div className="mt-auto pt-8">
            <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 flex gap-4 items-start">
              <div className="w-6 h-6 rounded-full bg-blue-900/50 text-blue-400 flex items-center justify-center text-xs font-bold mt-0.5">
                i
              </div>
              <p className="text-xs text-gray-400 leading-relaxed">
                {t("issueTicket.ticketInfo")}
              </p>
            </div>
          </div>
        </div>
